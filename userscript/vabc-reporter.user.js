...
// @version      3.0
// @description  AtCoderの提出詳細ページで、その問題が開催中のnagotch_virtualコンテストに含まれる場合のみ「報告」ボタンを表示します。ボタンを押すと提出結果を報告します（自動報告なし）。
// @author       traP
...
(function () {
  ...
  // ===== 設定 =====================================================
  // nagotch_virtual サーバーのURL（デプロイ先に合わせて変更）
  const NAGOTCH_VIRTUAL_API = 'http://localhost:3000';
  // ================================================================
  ...
  GM_xmlhttpRequest({
    method: 'GET',
    url: `${NAGOTCH_VIRTUAL_API}/api/contests/active-problems`,
    onload: (res) => {
      try {
        ...
        console.error('[nagotch_virtual] active-problems 取得失敗', e);
      }
    },
    onerror: () => console.error('[nagotch_virtual] active-problems 接続失敗（NAGOTCH_VIRTUAL_API / @connect を確認）'),
  });

  // ---- 手動報告ボタン（左下） ----
  function addManualButton() {
    const btn = document.createElement('button');
    btn.textContent = 'nagotch_virtualに報告';
    ...
    btn.addEventListener('click', () => {
      ...
      if (!data.atcoderId || !data.problemId || !data.submissionId || !data.result || !FINAL.includes(data.result)) {
        ...
        console.warn('[nagotch_virtual] 報告: 情報取得失敗', data);
        return;
      }
      flash(btn, '報告中...', '#6366f1');
      reportToNagotchVirtual(
        data,
  ...
  function flash(btn, text, color) {
    ...
    setTimeout(() => { btn.textContent = 'nagotch_virtualに報告'; btn.style.background = '#6366f1'; }, 2000);
  }
  ...
  function reportToNagotchVirtual(payload, onOk, onErr) {
    GM_xmlhttpRequest({
      method: 'POST',
      url: `${NAGOTCH_VIRTUAL_API}/api/submissions`,
      headers: { 'Content-Type': 'application/json' },
      data: JSON.stringify(payload),
      onload: (res) => {
        console.log('[nagotch_virtual] 報告完了', res.status, res.responseText);
        if (res.status >= 200 && res.status < 300) onOk && onOk();
        else onErr && onErr();
      },
      onerror: () => { console.error('[nagotch_virtual] 報告失敗'); onErr && onErr(); },
    });
  }
})();
